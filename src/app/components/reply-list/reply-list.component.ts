...
export class ReplyListComponent implements OnInit, OnDestroy {
    ...
    private getReply() {
        ...
        this.network.getReply(this.document._id)
            .subscribe(result => {
                if (result.result === true) {
        ...
        this.network.removeReply(this.account.accessToken, replyId)
            .subscribe(result => {
                if (result.result === true) {
                    ...
                    console.log(result.payload);
                    let temp: Reply[];
                    temp = this.replyList.filter(filtering => {
                        return filtering._id === replyId ? false : true;
                    });
                    temp.push(result.payload);
                    this.replyList = [...temp];
                } else {
                    // TODO
